...
def test_default_instatiation():
    ...
    _ = FokkerPlank1D(D, dt, dx, xArray)
...
def test_input_error_handling():
    ...
    with pytest.raises(ValueError):
        _ = FokkerPlank1D(D, dt, dx, xArray, diff_scheme=error_config)
...
def test_diffusion_matrix_initialization(init_config):
    ...
    # Act
    integrator = FokkerPlank1D(D, dt, dx, xArray, boundary_cond=boundaryCond)
