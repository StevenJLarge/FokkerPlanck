...
def check_CFL_error_for_invalid_init_parameters():
    ...
    force_param = [1.0]
    local_dx = 0.001
    fpe = FokkerPlank1D(D, dt, local_dx, x_array)
...
def test_dynamics_preserve_normalization(boundary_cond):
    ...
    n_steps = 100
    init_var = 1 / 64
    fpe = FokkerPlank1D(D, dt, dx, x_array, boundary_cond=boundary_cond)
...
def check_wave_speed_constant_force(boundary_cond):
    ...
    for _ in range(n_steps):
        fpe.advection_update(force_param, ff.constant_force)
